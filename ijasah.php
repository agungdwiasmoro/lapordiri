    ...
    <div class="row">
          ...
          <?php
            ...
            if(!in_array($ext,$ekstensi) ) {
                echo "<script>alert ('Format Tidak Didukung'); document.location='ijasah.php' </script>"; 
            }else{
              if($ukuran < 1044070){		
                $file = $rand.'_'.$filename;
                move_uploaded_file($_FILES['file']['tmp_name'], 'assets/'.$rand.'_'.$filename);
                mysqli_query($koneksi, "UPDATE user SET  ijasah='$file' WHERE id_user='$id'");
                    echo "<script>alert ('Data berhasil ditambahkan'); document.location='berkas.php' </script>"; 
              }else{
                    echo "<script>alert ('Data gagal ditambahkan'); document.location='ijasah.php' </script>"; 
              }
            }
          }
