...
<main class="content">
    ...
    <div class="row">
          ...
          <form action="" method="post" enctype="multipart/form-data" target="_self">
            <div class="card-body">
              <div class="mb-3">
                <label class="form-label fw-bold">PAS FOTO<span class="text-danger"> (LATAR BELAKANG MERAH)</span></label>
          ...
          <?php
            ...
            if(!in_array($ext,$ekstensi) ) {
                echo "<script>alert ('Format Tidak Didukung'); document.location='foto.php' </script>"; 
            }else{
              if($ukuran < 1044070){		
                $file = $rand.'_'.$filename;
                move_uploaded_file($_FILES['file']['tmp_name'], 'assets/'.$rand.'_'.$filename);
                mysqli_query($koneksi, "UPDATE user SET  pas_foto='$file' WHERE id_user='$id'");
                    echo "<script>alert ('Data berhasil ditambahkan'); document.location='berkas.php' </script>"; 
              }else{
                    echo "<script>alert ('Data gagal ditambahkan'); document.location='foto.php' </script>"; 
              }
            }
          }
